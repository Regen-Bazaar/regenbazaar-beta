...
///         ERC-2981 royalty) are paid to the listing's `beneficiary` (the NGO). Each listing carries its
///         own price (set per impact). Payment in native CELO (currency = address(0)) or an ERC-20.
/// @dev    Minimal, escrow-based, no upgradeability. checks-effects-interactions + ReentrancyGuard.
contract RegenMarketplace is AccessControl, ReentrancyGuard, IERC1155Receiver {
    ...
    struct Listing {
        ...
        address currency; // NATIVE or an ERC-20
        bool active;
    }
    ...
    function list(uint256 tokenId, uint256 amount, uint256 pricePerEdition, address currency, address beneficiary)
        external
        nonReentrant
        returns (uint256 listingId)
    {
        if (amount == 0 || pricePerEdition == 0 || beneficiary == address(0)) revert BadParams();
        trwi.safeTransferFrom(msg.sender, address(this), tokenId, amount, ""); // pull into escrow
    ...
    /// @notice Buy `amount` editions from a listing. Pays fee + ERC-2981 royalty + beneficiary, then
    ///         delivers the editions to the buyer.
    function buy(uint256 listingId, uint256 amount) external payable nonReentrant {
        ...
        // ---- split: platform fee, ERC-2981 royalty, remainder to beneficiary ----
        uint256 fee = (total * feeBps) / BPS;
        (address royaltyReceiver, uint256 royalty) = _royalty(l.tokenId, total);
        if (fee + royalty > total) royalty = 0; // safety: never overpay
    ...
    /// @notice Cancel a listing and return escrowed editions to the seller (seller or admin).
    ...
    function _royalty(uint256 tokenId, uint256 salePrice) internal view returns (address receiver, uint256 amount) {
